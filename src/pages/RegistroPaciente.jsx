...
export default function RegistroPaciente() {
  ...
  function validar() {
    ...
    const pesoNormalizado = String(form.peso).replace(",", ".")
    const tallaNormalizada = String(form.talla).replace(",", ".")

    const pesoNum = Number(pesoNormalizado)
    const tallaNum = Number(tallaNormalizada)

    if (!form.peso.trim()) {
      mostrarError("peso", "Ingresa el peso")
      valido = false
    } else if (isNaN(pesoNum) || pesoNum < 2 || pesoNum > 40) {
      mostrarError("peso", "Ingresa un peso realista entre 2 y 40 kg. Ejemplo: 16.5")
      valido = false
    }

    if (!form.talla.trim()) {
      mostrarError("talla", "Ingresa la talla")
      valido = false
    } else if (tallaNum >= 3) {
      mostrarError("talla", "La talla debe ingresarse en metros, no en centímetros. Ejemplo: 103 cm debe registrarse como 1.03")
      valido = false
    } else if (isNaN(tallaNum) || tallaNum < 0.45 || tallaNum > 1.50) {
      mostrarError("talla", "Ingresa una talla en metros entre 0.45 y 1.50. Ejemplo: 1.03")
      valido = false
    }
 ...
 async function handleSubmit(e) {
    ...
    const pacienteData = {
      ...
      dni: form.dni.trim(),
      sexo: form.sexo,
      peso: Number(pesoNormalizado),
      talla: Number(tallaNormalizada),
      departamento: form.departamento,
    }

    try {
      await api.post("/api/pacientes/", pacienteData)
      ...
      if (error.response?.data?.detail) {
        ...
        if (Array.isArray(detalle)) {
          mostrarError("general", detalle.map(e => e.msg).join(" | "))
        } else {
          mostrarError("general", detalle)
        }
      } else {
        mostrarError("general", "No se pudo registrar el paciente")
      }
    }
  }
        ...
        <form id="formPaciente" onSubmit={handleSubmit} autoComplete="off" className="flex flex-col flex-1 space-y-4">
          <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1">Talla (m) <span className="text-red-500">*</span></label>
                <input name="talla" type="text" placeholder="Ej: 0.85" value={form.talla} onChange={handleChange} className={inputClass} />
