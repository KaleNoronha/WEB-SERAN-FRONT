...
import { usuarios as usuariosIniciales } from "../data/mock"
...
export default function GestionUsuarios() {
  const [usuarios, setUsuarios] = useState(usuariosIniciales.filter(u => u.rol === "doctor"))
  ...
  function handleInvitar(e) {
    e.preventDefault()
    if (!emailInvitar) { mostrarErrorInvitar("Ingresa un correo electrónico."); return }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailInvitar)) { mostrarErrorInvitar("Ingrese un correo válido."); return }
    if (usuarios.find(u => u.email === emailInvitar)) { mostrarErrorInvitar("Este correo ya está habilitado en el sistema."); return }
    setUsuarios(prev => [...prev, {
      id: Date.now(), nombre: "Pendiente", apellido: "Pendiente",
      email: emailInvitar, rol: "doctor", estado: "pendiente",
      fechaInvitacion: new Date().toLocaleDateString("es-PE"), fechaRegistro: null
    }])
  ...
  function handleDesactivar() {
    setUsuarios(prev => prev.map(u => u.id === modalDesactivar.id ? { ...u, estado: "inactivo" } : u))
    mostrarMensaje("ok", `Doctor ${modalDesactivar.email} deshabilitado correctamente.`)
    setModalDesactivar(null)
  }

  function handleReactivar() {
    setUsuarios(prev => prev.map(u => u.id === modalReactivar.id ? { ...u, estado: "activo" } : u))
