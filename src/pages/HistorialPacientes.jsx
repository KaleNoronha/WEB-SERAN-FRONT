...
async function cargarPacientes() {
  try {
    ...
    const pacientesMapeados = response.data.map(p => ({
      ...
      ultimoRiesgo: p.ultimo_riesgo
        ? String(p.ultimo_riesgo).toLowerCase()
        : null,
    }))
  ...
  const pacientesConIndice = pacientesLista.map((p, i) => ({ ...p, indice: i + 1 }))
          ...
          <div className="fixed inset-0 z-50">
            ...
            <div className="fixed top-0 right-0 bottom-0 w-[380px] bg-white shadow-2xl flex flex-col z-10">
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div><p className="text-xs text-slate-400 mb-1">Nombres</p><FiltroInput placeholder="Nombres" value={draftNombres} onChange={setDraftNombres} opciones={pacientesLista.map(p => p.nombres)} /></div>
                <div><p className="text-xs text-slate-400 mb-1">Apellidos</p><FiltroInput placeholder="Apellidos" value={draftApellidos} onChange={setDraftApellidos} opciones={pacientesLista.map(p => p.apellidos)} /></div>
                ...
                <div><p className="text-xs text-slate-400 mb-1">DNI</p><FiltroInput placeholder="DNI" value={draftDni} onChange={setDraftDni} opciones={pacientesLista.map(p => p.dni)} /></div>
                <div>
                ...
                <div><p className="text-xs text-slate-400 mb-1">Peso (kg)</p><FiltroInput placeholder="Peso" value={draftPeso} onChange={setDraftPeso} opciones={pacientesLista.map(p => p.peso)} /></div>
                <div><p className="text-xs text-slate-400 mb-1">Talla (cm)</p><FiltroInput placeholder="Talla" value={draftTalla} onChange={setDraftTalla} opciones={pacientesLista.map(p => p.talla)} /></div>
                ...
                <div><p className="text-xs text-slate-400 mb-1">Evaluaciones</p><FiltroInput placeholder="Evaluaciones" value={draftEvaluaciones} onChange={setDraftEvaluaciones} opciones={pacientesLista.map(p => String(p.evaluaciones ?? 0))} /></div>
                <div>
